...
run_resolution() { # <arch>
  ...
  out="$(MOCK_UNAME_M="$arch" bash -c '
    ...
    uname() { printf "%s\n" "$MOCK_UNAME_M"; }
    RELEASE_BASE_URL="https://downloads.example/releases/download/v1.0.0-agent-beta.2"
    '"$BLOCK"'
    printf "%s\n" "$BINARY_URL"
  ' 2>&1)" || code=$?
  code="${code:-0}"
...
run_resolution "x86_64" "https://downloads.example/releases/download/v1.0.0-agent-beta.2/techfusion-agent-linux-x86_64"
run_resolution "amd64"  "https://downloads.example/releases/download/v1.0.0-agent-beta.2/techfusion-agent-linux-x86_64"
run_resolution "aarch64" "https://downloads.example/releases/download/v1.0.0-agent-beta.2/techfusion-agent-linux-aarch64"
run_resolution "arm64"  "https://downloads.example/releases/download/v1.0.0-agent-beta.2/techfusion-agent-linux-aarch64"
