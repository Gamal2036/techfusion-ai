...
const pages = [
  ...
  { label: 'Enrollment', href: '/dashboard/settings/enrollment', icon: Key },
...
export function CommandPalette({ open, onClose }: CommandPaletteProps) {
  ...
  return (
    <div ref={paletteRef} className="fixed inset-0 z-50">
      <div className="fixed inset-0 bg-black/60 backdrop-blur-sm" onClick={onClose} />
      <div className="fixed left-[50%] top-[50%] translate-x-[-50%] translate-y-[-50%] w-[90vw] max-w-[560px]">
        <Command className="rounded-2xl border border-border bg-popover/95 backdrop-blur-2xl shadow-dialog overflow-hidden">
          <div className="flex items-center border-b border-border px-4">
            <CommandIcon className="h-4 w-4 text-text-muted mr-3 shrink-0" />
            <Command.Input
              placeholder="Search pages..."
              className="flex-1 h-12 bg-transparent text-sm text-foreground placeholder:text-input-placeholder outline-none border-0"
            />
            <kbd className="px-1.5 py-0.5 rounded bg-surface-muted text-[10px] text-text-muted">ESC</kbd>
          </div>
              ...
              {pages.map((page) => {
                ...
                return (
                  <Command.Item
                    ...
                    className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-text-secondary cursor-pointer aria-selected:bg-primary/15 aria-selected:text-primary-400 transition-colors"
                  >
