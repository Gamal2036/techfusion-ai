...
export default function DeviceDetailPage() {
  ...
  useEffect(() => {
    if (device?.lastSeenAt) {
      setLastSeenAt(device.lastSeenAt);
    }
  }, [device?.lastSeenAt]);
  ...
  if (loading && !device) {
    return (
      <div className="flex items-center justify-center h-64">
        <Activity className="h-6 w-6 text-text-disabled animate-pulse" />
        <span className="ml-3 text-sm text-text-disabled">Loading device data...</span>
      </div>
    );
  }
  ...
  if (!device) {
    return (
      <GlassPanel intensity="light" className="p-12 text-center">
        <Monitor className="h-12 w-12 text-text-disabled mx-auto mb-4" />
        <h3 className="text-lg font-medium text-text-secondary">Device not found</h3>
        <Link href="/dashboard/device-health" className="text-sm text-primary hover:underline mt-2 inline-block">
  ...
  const chartData = metrics
    .map((m) => {
      const time = formatMetricTimestamp(m.recordedAt);
      if (!time) return null;
      return {
        time,
    ...
    .filter(Boolean) as { time: string; cpu: number; ram: number; temp: number | null }[];
    ...
    <div className="space-y-6 animate-fade-in">
        ...
        <div className="flex-1">
          ...
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-1 text-xs text-text-disabled">
            ...
            <span className="flex items-center gap-1">
              ...
              Last seen: {formatDeviceLastSeen(effectiveLastSeen)}
            </span>
          </div>
        </div>
      </div>
      ...
      <GlassPanel intensity="light" className="p-5">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-medium text-text-primary">Metrics History</h3>
            ...
            {[15, 30, 60, 120].map((m) => (
              <button
                ...
                className={`px-2.5 py-1 rounded-lg text-xs transition-all ${
                  timeRange === m
                    ? 'bg-primary-600/20 text-primary'
        ...
        {chartData.length > 0 ? (
          ...
          <div className="h-[300px] flex items-center justify-center text-text-disabled text-sm">
      ...
      {/* System Info */}
      <GlassPanel intensity="light" className="p-5">
        <h3 className="text-sm font-medium text-text-primary mb-4">System Information</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {[
            { label: 'CPU', value: device.cpuModel || 'CPU information unavailable', icon: Cpu },
            {
              label: 'Cores',
              value: (() => {
                const physical = device.cpuCores;
                const logical = device.cpuLogical;
                ...
                return 'Core information unavailable';
              })(),
              icon: Cpu,
            },
            ...
            return (
              <div key={item.label} className="flex items-start gap-3 rounded-xl border border-border-subtle bg-surface p-3">
                <Icon className="h-4 w-4 text-text-disabled mt-0.5" />
                <div>
                  <p className="text-xs text-text-disabled">{item.label}</p>
