...
mod tests {
    ...
    #[tokio::test]
    async fn test_pending_security_scans_send_bearer_header() {
        ...
        let result = client
            .get_pending_security_scans("device-token-abc", "dev-123")
            .await;
        let request = server.await.expect("server task");

        assert!(result.is_ok());
        let result = result.unwrap();
        assert!(result.is_empty(), "expected empty scan list");

        assert!(
            request.starts_with("GET /security/pending/dev-123 HTTP/1.1"),
        ...
        let auth_header = request
            ...
            .expect("pending-security request must carry an Authorization header");
        assert_eq!(
