...
| Network discovery + view | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ❌ | PRO | FUNCTIONAL (Linux) | Windows network module (`05`); Cybersecurity/device real-device integration certified `V1-STAGE-02-SUB-01A` (CYBER-01 CLOSED); **Network evidenced by `NET-00` (2026-08-12) — audit-complete, NOT COMPLETE; P0 NET-A01: agent discovery ships default-disabled (`TF_NETWORK_DISCOVERY=false`, `config.rs:80`) and installer never sets it → stock agent never scans (likely root cause of known real-device Network failure); P2 NET-A02 stale-pending scans, NET-A03 duplicate scan rows, NET-A04 no Web terminal-state/error banner, NET-A07 unvalidated agent ingress — see `NET-00` report; next = NET-01A**; org-wide `NetworkDevice` pool, unassigned scans claimable by org agents, diagnostics from API host — documented gaps |
