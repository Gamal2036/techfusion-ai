...
|-------|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|
| Auth (signup/login/logout/refresh) | ✅ | ✅ | User, RefreshToken, OrganizationMember | JWT, membership gate, throttles | ✅ | DTOs | ✅ `auth.spec.ts`, `session-refresh.spec.ts`, `slug-collision.spec.ts` | ✅ login/signup | CERTIFIED | `ACC-SEC-02D2A`: refresh tokens stored as SHA-256 verifiers (`rt:v1:<sha256-hex>`) with legacy-plaintext atomic upgrade (see §6); stable `sessionId` + additive `sid` claim + server-observed metadata — `refresh-token-hardening.spec.ts` (20 proofs) |
