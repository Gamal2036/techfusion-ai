...
- **NEXT stage**: **`NET-01A — Real-Device Network Evidence Fix (P0)`** — enable agent network discovery by default + set `TF_NETWORK_DISCOVERY=true` in `install-linux.sh`, report discovery capability to the server, then manual real-device Network evidence; then `NET-01B` (scan lifecycle/data truth: stale-pending sweep, single scan row per trigger, Web terminal-state/error banner, agent ingress DTO) and `NET-01C` (per-device attribution, unassigned-scan semantics, diagnostics vantage point). `V1-STAGE-02-SUB-02` (Deployment Reliability & CD Repairs; T1-T4 + `METRICS_AUTH_TOKEN` Helm wiring) remains preserved as a later substage.
