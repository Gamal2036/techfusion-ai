...
- **Status: MANUAL CERTIFICATION REQUIRED** — automated checks cannot certify real-agent↔real-stack E2E (telemetry render, presence stop/restart transitions, same-Device reconnect, exact-Device security data, network discovery). The installed agent binary must be **rebuilt/reinstalled** (it predates the Bearer header), then the operator executes the §9 manual checklist — incl. the CYBER-01 retest (`GET /security/pending/:deviceId` → 200; scan reaches a truthful terminal state) — and returns evidence before the stage closes.
- Report: `V1-STAGE-02-SUB-01A_REAL_DEVICE_PRODUCT_INTEGRATION_REPORT.md`.
- **NEXT substage**: `V1-STAGE-02-SUB-02` — Deployment Reliability & CD Repairs (preserved; T1-T4 + `METRICS_AUTH_TOKEN` Helm wiring), gated on completion of the SUB-01A manual real-device certification.
...
**V1-STAGE-02 — Device Identity & Presence Reliability + Deployment Reliability & CD Repairs** (SUB-01 complete; SUB-01A certification in progress — manual real-device gate pending; SUB-02 = Deployment/CD T1-T4).

## NEXT EXACT SUBSTAGE

**V1-STAGE-01 is CLOSED** — all security/tenancy/credential items (S1–S5 + secrets hygiene) are done. **`V1-STAGE-02-SUB-01` (Enrollment, Token & Device-Link Reliability) is COMPLETE. `V1-STAGE-02-SUB-01A` (Real Device Product Integration Certification) is in progress — automated certification + defect fixes complete (incl. CYBER-01: Cybersecurity Web terminal-state machine + failed-scan API exposure; agent source verified Bearer-authenticated), MANUAL CERTIFICATION REQUIRED** (operator REBUILDS/REINSTALLS the agent, then executes the §9 real-device checklist incl. the CYBER-01 retest; stage closes on evidence). **Next substage after SUB-01A evidence: `V1-STAGE-02-SUB-02` — Deployment Reliability & CD Repairs** (T1–T4; includes wiring `METRICS_AUTH_TOKEN` into Helm / Prometheus ServiceMonitor per the SUB-05 residual).
