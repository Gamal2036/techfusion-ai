...
- **3 integration defects fixed**: **SEC-1** `POST /devices/security-report` was fail-open (200 + error body for invalid/revoked credential, which the agent treated as success) → now 401 fail-closed, consistent with DeviceTokenGuard (agent already re-registers on 401); **NET-1** Network page polled a non-existent `/api/network/scans` endpoint — completion detection now uses real `GET /network/scans` data; **NET-2** unknown-MAC sentinel `00:00:00:00:00:00` rendered as a real MAC → now `-`. `AGENT CHANGE: NONE`, `MIGRATION: NONE`.
- Verified: api-gateway 58 suites / 994 tests; web 35 suites / 791; worker 8 / 80; agent 78; `pnpm lint` + `pnpm build` green; `scripts/ci-v1-gate.sh` 19/19 PASS (incl. migration validation + worker schema sync + secret scan — NO SECRETS DETECTED).
- **Status: MANUAL CERTIFICATION REQUIRED** — automated checks cannot certify real-agent↔real-stack E2E (telemetry render, presence stop/restart transitions, same-Device reconnect, exact-Device security data, network discovery). Operator must execute the §9 manual checklist and return evidence before the stage closes.
