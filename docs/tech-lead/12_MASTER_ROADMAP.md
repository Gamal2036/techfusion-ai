...
- **NEXT stage**: **`NET-00 — Network Evidence & Current-State Audit`** — real-device Network evidence (manual §9 step 8), then per-device attribution, unassigned-scan semantics, diagnostics vantage point. `V1-STAGE-02-SUB-02` (Deployment Reliability & CD Repairs; T1-T4 + `METRICS_AUTH_TOKEN` Helm wiring) remains preserved as a later substage.
...
**V1-STAGE-02 — Device Identity & Presence Reliability + Deployment Reliability & CD Repairs** (SUB-01 complete; SUB-01A Cybersecurity certification COMPLETE — CYBER-01 CLOSED for V1; next = `NET-00` Network Evidence & Current-State Audit; SUB-02 = Deployment/CD T1-T4 preserved).
