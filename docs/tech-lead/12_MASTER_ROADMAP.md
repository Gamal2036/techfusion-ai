...
**Completed: V1-STAGE-02-SUB-01A — Real Device Product Integration Certification** (2026-08-11):
- **Integration boundaries certified**: traced the real Agent→Device Identity→Presence/Telemetry→Dashboard, Cybersecurity, and Network paths. All three surfaces render REAL_AGENT_DATA/SERVER_DERIVED only — zero hardcoded/demo/mock device values in web production pages, org/device identity always server-derived (DeviceTokenGuard / membership), no client-authoritative orgId/deviceId, UNKNOWN preferred over fabrication. `GET /security/scans/detail/:scanId` verified NOT shadowed by `:deviceId`.
- **5 integration defects/root causes fixed**: **SEC-1** `POST /devices/security-report` was fail-open (200 + error body for invalid/revoked credential, which the agent treated as success) → now 401 fail-closed, consistent with DeviceTokenGuard (agent already re-registers on 401); **NET-1** Network page polled a non-existent `/api/network/scans` endpoint — completion detection now uses real `GET /network/scans` data; **NET-2** unknown-MAC sentinel `00:00:00:00:00:00` rendered as a real MAC → now `-`; **CYB-1** Cybersecurity Web polling had no terminal-state model — `useSecurity.ts` now implements `idle`/`triggering`/`running`/`completed`/`failed`/`timeout` (completed/failed stop polling; 120 s backstop → honest timeout/retry; 401/403 + unexpected failures stop polling with an honest error banner — never infinite loading); **CYB-2** `GET /security/latest/:deviceId` now returns the latest terminal scan (`completed|failed`) and exposes the scan `error`, backward-compatibly. `AGENT CHANGE: TEST-ONLY` (source at `client.rs:484` already sent the Bearer credential on the pending-security request; the installed binary was STALE — subsequently rebuilt/reinstalled and the manual retest PASSED, CYBER-01 CLOSED), `MIGRATION: NONE`.
- Verified: api-gateway 58 suites / 997 tests (incl. +3 `getLatestScan` regression tests); web 36 suites / 797 (incl. +6 `use-security.spec.ts`); worker 8 / 80; agent 79 (incl. +1 pending-security Bearer-header test); `pnpm lint` + `pnpm build` green; `scripts/ci-v1-gate.sh` 19/19 PASS (incl. migration validation + worker schema sync + secret scan — NO SECRETS DETECTED).
- **Status: MANUAL CERTIFICATION PASS — CYBER-01 CLOSED for V1** — operator executed the §9 real-device checklist on a real Ubuntu host with the **REBUILT/REINSTALLED** agent (`/usr/local/bin/techfusion-agent`): persistent identity/credential preserved, Dashboard recognizes the real enrolled device, Cybersecurity page stable, real-device scan flow completes to a truthful terminal state, no permanent-loading/timeout (previous behavior traced to the stale deployed binary). Evidence: report §9.1. **CYBER-01 (Cybersecurity real-device reliability) is CLOSED; Cybersecurity is preserved as the V1 stable baseline** and future Cybersecurity capabilities stay extensible under the Module Extensibility Contract (`15`).
- Report: `V1-STAGE-02-SUB-01A_REAL_DEVICE_PRODUCT_INTEGRATION_REPORT.md`.
- **NEXT stage**: **`NET-01A — Real-Device Network Evidence Fix (P0)` — IMPLEMENTED_PENDING_REAL_DEVICE_CERTIFICATION (2026-08-12)** — agent network discovery is now **ENABLED by default** (`config.rs:80` `TF_NETWORK_DISCOVERY=true`), `install-linux.sh` writes `TF_NETWORK_DISCOVERY=true` (explicit operator opt-out preserved), enabled+identified agents enter `poll_pending_discovery_commands` (device Bearer auth unchanged), and the **Web-vs-Agent auth/polling boundary is closed** — Web trigger = user JWT `POST /network/discovery/trigger`, Web status polling = user JWT `GET /network/scans` only (never `/network/discovery/pending`; device token never sent to the browser), `useStartDiscovery` terminal-state machine (completed/failed/timeout, NET-A04 resolved). V1 gate 19/19 PASS (API 999 / web 802 / worker 80 / agent 84). **Remaining: manual real-device Network evidence (§18 of the NET-00 report), then close NET-A01.** Then `NET-01B` (scan lifecycle/data truth: stale-pending sweep, single scan row per trigger, agent ingress DTO) and `NET-01C` (per-device attribution, unassigned-scan semantics, diagnostics vantage point). `V1-STAGE-02-SUB-02` (Deployment Reliability & CD Repairs; T1-T4 + `METRICS_AUTH_TOKEN` Helm wiring) remains preserved as a later substage.
...
**V1-STAGE-02 — Device Identity & Presence Reliability + Deployment Reliability & CD Repairs** (SUB-01 complete; SUB-01A Cybersecurity certification COMPLETE — CYBER-01 CLOSED for V1; NET-00 Network audit COMPLETE; **NET-01A P0 fix IMPLEMENTED_PENDING_REAL_DEVICE_CERTIFICATION**; SUB-02 = Deployment/CD T1-T4 preserved).

## NEXT EXACT SUBSTAGE

**V1-STAGE-01 is CLOSED** — all security/tenancy/credential items (S1–S5 + secrets hygiene) are done. **`V1-STAGE-02-SUB-01` (Enrollment, Token & Device-Link Reliability) is COMPLETE. `V1-STAGE-02-SUB-01A` (Real Device Product Integration Certification) is COMPLETE for Cybersecurity — CYBER-01 CLOSED for V1**: automated certification + defect fixes (Cybersecurity Web terminal-state machine + failed-scan API exposure; agent source verified Bearer-authenticated) plus **MANUAL CERTIFICATION PASS** on a real Ubuntu host with the rebuilt/reinstalled agent (evidence in report §9.1); Cybersecurity is preserved as the V1 stable baseline. **`NET-00` (Network Evidence & Current-State Audit) is COMPLETE (2026-08-12)**: audit-only, full Network path traced in current code; security boundary certified intact; discovery data is real agent data end-to-end; **P0 NET-A01 proven** (agent discovery default-disabled `config.rs:80` + installer omits `TF_NETWORK_DISCOVERY` → stock agent never scans) plus P2 NET-A02/A03/A04/A07 — Network is **NOT COMPLETE**, evidence in `NET-00_NETWORK_EVIDENCE_CURRENT_STATE_REPORT.md`. **`NET-01A` (P0 fix) is IMPLEMENTED (2026-08-12)**: agent discovery ENABLED by default + installer `TF_NETWORK_DISCOVERY=true` (opt-out preserved), enabled+identified agent polling gate (device Bearer auth unchanged), and the **Web-vs-Agent auth/polling boundary closed** (Web trigger = user JWT `POST /network/discovery/trigger`; Web status polling = user JWT `GET /network/scans` only — never `/network/discovery/pending`; device token never sent to the browser; `useStartDiscovery` terminal-state machine completed/failed/timeout, NET-A04 resolved). Focused Web-vs-Agent regression tests + V1 gate 19/19 PASS (API 999 / web 802 / worker 80 / agent 84). **Remaining: manual real-device Network evidence (§18 of the NET-00 report) to close NET-A01**, then `NET-01B` (scan lifecycle/data truth) and `NET-01C` (attribution/diagnostics). `V1-STAGE-02-SUB-02` (Deployment Reliability & CD Repairs; T1–T4; includes wiring `METRICS_AUTH_TOKEN` into Helm / Prometheus ServiceMonitor per the SUB-05 residual) remains preserved as a later substage.
